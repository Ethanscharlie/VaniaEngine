...
Runner::Runner(const nlohmann::json& gameData, SDL_Renderer* renderer)
  ...
  // lua.new_usertype<EntityDef>("EntityDef", "width", &EntityDef::width, "height", &EntityDef::height);
  //
  // lua.new_usertype<RuntimeEntity>("Entity", "x", &RuntimeEntity::x, "y", &RuntimeEntity::y, "def",
...
void Runner::render() {
  ...
  //
  // for (auto& entity : entities) {
  //   SDL_SetRenderDrawColor(renderer, entity.entityDef.r, entity.entityDef.g, entity.entityDef.b, 255);
  //   SDL_FRect rect = {(float)entity.x, (float)entity.y, (float)entity.entityDef.width,
  //   (float)entity.entityDef.height}; SDL_RenderFillRect(renderer, &rect);
  //
