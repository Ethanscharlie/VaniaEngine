...
void Runner::render() {
  ...
  for (auto &entity : gameDataCopy.worldData.entities) {
    ...
    if (def.imageMode) {
      SDL_Texture *texture =
          AssetManager::getInstance().get(renderer, root / def.image);
      SDL_FRect srcRect = {
          def.imageCol,    //
          def.imageRow,    //
          def.imageWidth,  //
      ...
      SDL_RenderTextureRotated(renderer, texture, &srcRect, &rect, entity.angle,
                               nullptr, SDL_FLIP_NONE);
    }
    //
    else {
      SDL_SetRenderDrawColor(
          renderer, entity.entityDefOverride.r, entity.entityDefOverride.g,
